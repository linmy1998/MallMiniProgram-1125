...
const addressData = computed(() => {
  return addressStore.selectedAddress || orderPre.value.userAddresses.find((v) => v.isDefault === 1)
})
...
<template>
  <scroll-view scroll-y class="viewport">
    ...
    >
      <view class="user"> {{ addressData.receiver }} {{ addressData.contact }} </view>
      <view class="address"> {{ addressData.fullLocation }} {{ addressData.address }} </view>
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    ...
    <view class="button" :class="{ disabled: addressData.value?.id }" @click="onOrderSubmit">
      提交订单
    </view>
  </view>
</template>
